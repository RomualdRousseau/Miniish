...
def repeat_n_func(buffer, func):
    for n in range(max(buffer.selblock, 1)):
        func()
    buffer.selblock = 0
...
def state_normal(buffer, c):
    state_normal_action(buffer, c) or state_normal_navigation(
        buffer, c
    ) or state_normal_enter_insert(buffer, c) or state_normal_enter_delete(
        buffer, c
    ) or state_normal_enter_copy(
        buffer, c
    )
...
def state_normal_navigation(buffer, c):
    if c == "h" or c == "left":
        ...
        repeat_n_func(buffer, lambda: move_cursor_previous_word(buffer, True))
        return True
    elif c == "B":
        repeat_n_func(buffer, lambda: move_cursor_previous_word(buffer, False))
        return True
    elif c == "g":
        buffer.state = "GOTO"
        return True
...
def state_normal_enter_delete(buffer, c):
    if c == "x" and len(buffer.buf) > 0:
        ...
        repeat_n_func(
            buffer,
            lambda: (
                f() for f in [record_current_step(buffer, c), delete_character(buffer)]
            ),
        )
        return True
    elif c == "d":
...
def state_delete(buffer, c):
    if c == "d":
        ...
        repeat_n_func(
            ...
            lambda: (
                f()
                for f in [record_current_step(buffer, c), delete_current_line(buffer)]
            ),
        )
    elif c == "$":
        buffer.state = "NORMAL"
